...
export default function Skills({ theme }) {
  ...
  return (
    <section id="skills" className={`py-20 px-4 ${theme === 'dark' ? 'bg-slate-900' : 'bg-slate-50'}`}>
      <div className="max-w-6xl mx-auto z-10 relative">
        ...
        >
          <h2 className={`text-4xl font-black mb-4 ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>Tech Stack</h2>
          <p className={`${theme === 'dark' ? 'text-slate-400' : 'text-slate-600'}`}>Tools I use to build</p>
        </motion.div>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {skillCategories.map((category, index) => (
            ...
            >
              <h3 className={`text-lg font-bold mb-4 ${theme === 'dark' ? 'text-cyan-400' : 'text-cyan-600'}`}>
                {category.title}
              </h3>
