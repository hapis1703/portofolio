...
export default function Hero({ theme }) {
  ...
  if (loading) {
    return (
      <section id="home" className={`min-h-screen flex items-center justify-center px-4 py-20 ${theme === 'dark' ? 'bg-slate-950' : 'bg-slate-50'}`}>
        <div className="text-center">
          <div className={`w-32 h-32 mx-auto mb-6 rounded-full animate-pulse ${theme === 'dark' ? 'bg-slate-800' : 'bg-slate-200'}`} />
          <div className={`h-8 w-64 mx-auto mb-4 rounded animate-pulse ${theme === 'dark' ? 'bg-slate-800' : 'bg-slate-200'}`} />
          <div className={`h-6 w-96 mx-auto rounded animate-pulse ${theme === 'dark' ? 'bg-slate-800' : 'bg-slate-200'}`} />
        </div>
      </section>
    );
  }
  ...
  return (
    <section id="home" className={`min-h-screen flex items-center justify-center px-4 py-20 relative ${theme === 'dark' ? 'bg-slate-950' : 'bg-slate-50'}`}>
      <div className="max-w-4xl mx-auto text-center z-10">
        ...
        >
          <h1 className={`text-5xl md:text-7xl font-black mb-4 ${theme === 'dark' ? 'bg-gradient-to-r from-cyan-300 to-purple-300' : 'bg-gradient-to-r from-cyan-600 to-purple-600'} bg-clip-text text-transparent`}>
            Hi, I'm Ahmad Hafizh 👋
          </h1>
        </motion.div>
