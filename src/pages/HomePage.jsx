...
export default function HomePage({ colors }) {
  ...
  if (loading) {
    return (
      <section
        className={`min-h-screen flex items-center justify-center px-4 py-20 ${colors.bg}`}
      >
        <div className="text-center">
          <div
            className={`w-32 h-32 mx-auto mb-6 rounded-full animate-pulse ${colors.card}`}
          />
          <div
            className={`h-8 w-64 mx-auto mb-4 rounded animate-pulse ${colors.card}`}
          />
          <div
            className={`h-6 w-96 mx-auto rounded animate-pulse ${colors.card}`}
          />
        </div>
      </section>
    );
  ...
  return (
    <section
      className={`min-h-screen flex items-center justify-center px-4 py-20 relative ${colors.bg}`}
    >
      <div className="max-w-4xl mx-auto text-center z-10">
        ...
        >
          <h1
            className={`text-5xl md:text-7xl font-black mb-4 bg-gradient-to-r ${colors.gradient} bg-clip-text text-transparent`}
          >
            Hi, I'm Ahmad Hafizh 👋
          </h1>
        </motion.div>
