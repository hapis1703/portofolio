...
export default function App() {
  const [isDark, setIsDark] = useState(
    localStorage.getItem("isDark") === "true" || true,
  );
  const [currentTheme, setCurrentTheme] = useState(
    localStorage.getItem("currentTheme") || "modern",
  );
  ...
  return (
    <Router>
      <div
        className={`min-h-screen font-sans selection:bg-${colors.primary} selection:text-white ${colors.bg} ${colors.text}`}
      >
        ...
        <main className="pt-16">
          <Routes>
            ...
            <Route
              path="/projects"
              element={<ProjectsPage colors={colors} />}
            />
